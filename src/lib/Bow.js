/*
LIBRARY MODIFIED BY HANPROGRAMER
1. Using the bow doesn't reduce it's durability
2. Added callback for when the arrow spawned
3. each arrow has unique identifier ('arrow_id')
4. Added callback for when the arrow landed
5. Added callback for when the bow state changed
*/
...
//const filterItems = (arr, query) => { return arr.filter(el => el.inst_id !== query) }
...
function CustomArrow() {
	this.spawn = function (params) {
		...
		this.entity = entity;
		this.id = params.arrow_id;
		Callback.invokeCallback("CustomArrowSpawn", this.entity,this.id);
  this.pos = Entity.getPosition(this.entity);
  this.inst_id = arrow_inst_id;
  arrow_inst_id+=1;
		return entity;
	}
	...
	this.onLand = function(){
	   Callback.invokeCallback("CustomArrowLand", this.entity,this.id);
	}
	this.onRemove = function(){
	   Callback.invokeCallback("CustomArrowRemoved", this.pos,this.id);
	}
}
